...
import { isValidToken, extractBearerToken } from "./auth.js";
...
async function handleEvents(url: URL): Promise<Response> {
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      try {
        unsubscribe = onNewMessage(code, (messageId) => {
...
Bun.serve({
  ...
  async fetch(req) {
    ...
    if (!isValidToken(extractBearerToken(req))) {
    ...
    try {
      if (url.pathname === "/register" && req.method === "POST") {
        ...
        await presence.register(body.code, body.project);
        return json({ ok: true });
      }
      ...
      if (url.pathname === "/heartbeat" && req.method === "POST") {
        ...
        const ok = await presence.heartbeat(body.code);
        return json({ ok });
      }
      ...
      if (url.pathname === "/who" && req.method === "GET") {
        ...
        const agents = await presence.listAgents(filter);
        return json({ agents });
      }
      ...
      if (url.pathname === "/reply" && req.method === "POST") {
        ...
        const inbox = await messages.peekMessages(body.from);
        const original = inbox.find((m) => m.id === body.message_id);
        if (!original) return json({ error: "message not found" }, 404);
        const msg = await messages.sendMessage(body.from, original.from, body.message, body.message_id);
        await messages.ackMessage(body.from, body.message_id);
        return json({ message: msg });
      }
      ...
      if (url.pathname === "/peek" && req.method === "GET") {
        const code = url.searchParams.get("code");
        if (!code) return json({ error: "code query param required" }, 400);
        const inbox = await messages.peekMessages(code);
      ...
      if (url.pathname === "/ack" && req.method === "POST") {
        ...
        const ok = await messages.ackMessage(body.code, body.message_id);
      ...
      if (url.pathname === "/ack_all" && req.method === "POST") {
        const body = await readJson(req);
        if (!body?.code) return json({ error: "code required" }, 400);
        const count = await messages.ackAll(body.code);
        return json({ count });
      }
